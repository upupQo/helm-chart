...
## Test：  
1.login spark master node：`minikube kubectl -- exec -it pod/spark-master-0 /bin/bash`  
2.read hdfs in spark-shell：

```shell
spark-shell
scala> var files=spark.read.format("text").load("hdfs://hadoop-hadoop-hdfs-nn-0.hadoop-hadoop-hdfs-nn.default:9000/")
# false measn no truncate
scala> files.show(false)
```

output maybe like this (if you have put hdfs file)：
